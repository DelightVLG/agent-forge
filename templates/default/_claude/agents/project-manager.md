...
description: Use PROACTIVELY when the user describes a feature, bug, or change request at a high level. Decomposes work into concrete task specs, creates GitHub Issues, and writes task files to .agent-memory/tasks/.
model: opus
...
1. Read `.agent-memory/project.md`. If missing/empty → tell the user to run `/init-project` and stop.
...
3. **Write task files** to `.agent-memory/tasks/<id>-<slug>.md` using the template below. IDs are zero-padded sequential (`001`, `002`...).
4. **Create GitHub Issues** via `gh issue create --title "..." --body-file .agent-memory/tasks/<id>-<slug>.md --label <app>`. Record the issue number in the task file.
5. **Report** to the user: list of task IDs + issue links + suggested execution order.
