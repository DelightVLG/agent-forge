---
name: web-dev
description: Implements web frontend tasks from .agent-memory/tasks/. Writes code AND tests in the same change. Invoke with a task ID.
model: sonnet
...
3. Implement inside `apps/web/`. If BE or Mobile changes are required, stop and flag.
...
7. For any UI change, verify a11y basics (labels, roles, keyboard nav) — see `.claude/skills/frontend/a11y.md`.
