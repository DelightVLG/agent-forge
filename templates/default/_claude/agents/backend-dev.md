...
5. Run `pnpm --filter backend test` locally. Fix failures.
6. Run linter/formatter as defined in project.md.
7. Commit with Conventional Commits. Reference the issue: `feat(backend): ... (#<issue>)`.
...
- Never touch `apps/frontend/` — if the task needs FE changes, flag it and stop.
