...
- [x] **Композиция из блоков.** Вместо набора монолитных темплейтов проект
      собирается на лету из `_base` + выбранных `_apps/*`. Поддержаны все 7
      комбинаций backend × web × mobile без дублирования файлов.
...
- [ ] **Живой preview шаблона.** При выборе структуры (backend / web / mobile)
      ...
      `mobile-only`, `minimal`) и краткое описание его содержимого.
